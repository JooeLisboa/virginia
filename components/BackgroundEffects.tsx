...
export function BackgroundEffects() {
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 overflow-hidden rounded-[2rem]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.18),_transparent_34%),linear-gradient(160deg,rgba(10,12,26,0.92),rgba(17,10,33,0.95)_35%,rgba(7,13,32,0.92)_100%)]" />
      ...
      {glows.map((glow, index) => (
        <motion.div
          ...
          className={cn(
            ...
            'h-44 w-44 sm:h-56 sm:w-56 bg-gradient-to-br',
            glow,
      ...
      <motion.div
        ...
        animate={reduceMotion ? undefined : { opacity: [0.2, 0.35, 0.2], scaleX: [1, 1.08, 1] }}
      ...
      <div className="absolute inset-0">
        {Array.from({ length: 12 }).map((_, index) => (
          <motion.span
            ...
            animate={reduceMotion ? undefined : { opacity: [0.15, 0.8, 0.15], scale: [0.9, 1.4, 0.9] }}
            transition={{ duration: 3.5 + (index % 4), repeat: Infinity, delay: index * 0.18 }}
          />
        ))}
      </div>
    </div>
